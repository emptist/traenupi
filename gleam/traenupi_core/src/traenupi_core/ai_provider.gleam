...
import traenupi_core/http.{
  type HttpError, type HttpRequest, NetworkError, StatusError, Timeout,
}
...
pub fn detect_provider(model: String, api_key: String) -> Provider {
  case string.contains(model, "llama") || string.contains(model, "mistral") {
    True -> ollama()
...
pub fn ask_with_fallback(
  ...
  case chat_completion(primary, model, messages, system_prompt) {
    ...
    Error(primary_error) ->
      case chat_completion(fallback, fallback_model, messages, system_prompt) {
